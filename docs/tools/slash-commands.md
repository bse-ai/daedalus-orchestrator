...
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from ForgeOrchestrator session logs.
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/forge-orchestrator`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
