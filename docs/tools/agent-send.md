---
summary: "Direct `forge-orchestrator agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `forge-orchestrator agent` (direct agent runs)

`forge-orchestrator agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `forge-orchestrator message --target`).
...
```bash
forge-orchestrator agent --to +15555550123 --message "status update"
...
forge-orchestrator agent --to +15555550123 --message "Summon reply" --deliver
