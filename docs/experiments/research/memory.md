...
read_when:
  - Designing workspace memory (~/.forge-orchestrator/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep ForgeOrchestrator integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.forge-orchestrator/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.forge-orchestrator/workspace` as canonical human-readable memory.
...
```
~/.forge-orchestrator/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in ForgeOrchestrator**, but keep a separable core library.

### Why integrate into ForgeOrchestrator?

- ForgeOrchestrator already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `forge-orchestrator memory recall "…" --k 25 --since 30d`
  - `forge-orchestrator memory reflect --since 7d`
...
Pragmatic take for `~/.forge-orchestrator/workspace`:
