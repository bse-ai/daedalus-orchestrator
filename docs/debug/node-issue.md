...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ForgeOrchestrator
...
Running ForgeOrchestrator via Node with `tsx` fails at startup with:

```
[forge-orchestrator] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `forge-orchestrator status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch forge-orchestrator.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node forge-orchestrator.mjs status` works on Node 25.
