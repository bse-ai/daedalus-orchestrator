...
description: "Complete field-by-field reference for ~/.forge-orchestrator/forge-orchestrator.json"
---
...
- Legacy single-account Baileys auth dir is migrated by `forge-orchestrator doctor` into `whatsapp/default`.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, ForgeOrchestrator assumes `anthropic` (deprecated).
...
- Override agent config root with `FORGE_ORCH_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or FORGE_ORCH_GATEWAY_PASSWORD
      allowTailscale: true,
      rateLimit: {
        maxAttempts: 10,
        windowMs: 60000,
...
- Legacy OAuth imports from `~/.forge-orchestrator/credentials/oauth.json`.
