...
## Quick check: `forge-orchestrator security audit`
...
- Tighten local perms (`~/.forge-orchestrator` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

ForgeOrchestrator is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
- **Pairing allowlists**: `~/.forge-orchestrator/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.forge-orchestrator/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.forge-orchestrator/credentials/oauth.json`
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.forge-orchestrator/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
4. **Re-run audit**
   - `forge-orchestrator security audit --deep` and confirm the report is clean.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `FORGE_ORCH_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), ForgeOrchestrator
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. ForgeOrchestrator verifies the identity by resolving the
