...
read_when:
  - You want ForgeOrchestrator to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables ForgeOrchestrator to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`forge-orchestrator onboard`) and `forge-orchestrator channels add` list optional channel plugins.
...
```bash
forge-orchestrator plugins install @forge-orchestrator/nostr
```

Use a local checkout (dev workflows):

```bash
forge-orchestrator plugins install --link <path-to-forge-orchestrator>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "forge-orchestrator@example.com",
        "lud16": "forge-orchestrator@example.com"
      }
    }
  }
}
