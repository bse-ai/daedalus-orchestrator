...
read_when:
  - Setting up Twitch chat integration for ForgeOrchestrator
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. ForgeOrchestrator connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
forge-orchestrator plugins install @forge-orchestrator/twitch
```

Local checkout (when running from a git repo):

```bash
forge-orchestrator plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `FORGE_ORCH_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "forge-orchestrator", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use FORGE_ORCH_TWITCH_ACCESS_TOKEN env var)
...
```bash
FORGE_ORCH_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
