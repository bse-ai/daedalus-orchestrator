...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.forge-orchestrator/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.forge-orchestrator/node.json` (node id + pairing token).
...
Path: `~/.forge-orchestrator/exec-approvals.json`
...
- Unix socket at `~/.forge-orchestrator/exec-approvals.sock` (0600).
