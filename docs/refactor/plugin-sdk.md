...
- Publish as `forge-orchestrator/plugin-sdk` (or export from core under `forge-orchestrator/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ForgeOrchestratorPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ForgeOrchestratorConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ForgeOrchestratorConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ForgeOrchestratorConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ForgeOrchestratorConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ForgeOrchestratorConfig): string;
  };
};
```
...
- Introduce `forge-orchestrator/plugin-sdk`.
- Add `api.runtime` to `ForgeOrchestratorPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `forge-orchestratorRuntime: ">=2026.2.0"`).
