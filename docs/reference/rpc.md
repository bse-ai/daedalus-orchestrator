...
ForgeOrchestrator integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- ForgeOrchestrator owns lifecycle when `channels.signal.autoStart=true`.
...
- ForgeOrchestrator spawns `imsg rpc` as a child process (legacy iMessage integration).
