---
read_when:
  - 你想在 ForgeOrchestrator 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 ForgeOrchestrator 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
forge-orchestrator plugins enable minimax-portal-auth  # 如果已加载则跳过
forge-orchestrator gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/forge-orchestrator/forge-orchestrator/tree/main/extensions/minimax-portal-auth)。
...
- 运行 `forge-orchestrator configure`
- 选择 **Model/auth**
- 选择 **MiniMax M2.1**
...
通过 `forge-orchestrator.json` 手动配置：
...
- 使用 `forge-orchestrator models list` 和 `forge-orchestrator models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `forge-orchestrator configure` 并选择 **MiniMax M2.1**，或
