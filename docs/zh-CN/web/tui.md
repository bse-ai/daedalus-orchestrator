...
- 开启投递：
  - `/deliver on`
  - 或设置面板
  - 或使用 `forge-orchestrator tui --deliver` 启动
...
- 确认智能体可以运行：`forge-orchestrator status` 和 `forge-orchestrator models status`。
...
- 选择器中没有智能体：检查 `forge-orchestrator agents list` 和你的路由配置。
