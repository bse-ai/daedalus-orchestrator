...
```bash
forge-orchestrator plugins install @forge-orchestrator/nextcloud-talk
```

本地检出安装（从 git 仓库运行时）：

```bash
forge-orchestrator plugins install ./extensions/nextcloud-talk
```
...
2. 在你的 Nextcloud 服务器上创建机器人：
   ```bash
   ./occ talk:bot:install "ForgeOrchestrator" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
3. 在目标房间设置中启用机器人。
...
- 批准方式：
  - `forge-orchestrator pairing list nextcloud-talk`
  - `forge-orchestrator pairing approve nextcloud-talk <CODE>`
