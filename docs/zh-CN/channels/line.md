---
read_when:
  - 你想将 ForgeOrchestrator 连接到 LINE
...
LINE 通过 LINE Messaging API 连接到 ForgeOrchestrator。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
