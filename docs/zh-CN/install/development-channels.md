...
当你使用 `--channel` **显式**切换渠道时，ForgeOrchestrator 还会对齐安装方式：

- `dev` 确保有一个 git checkout（默认 `~/forge-orchestrator`，可通过 `FORGE_ORCH_GIT_DIR` 覆盖），
...
当你使用 `forge-orchestrator update` 切换渠道时，ForgeOrchestrator 还会同步插件来源：
