...
summary: "`forge-orchestrator onboard` 的 CLI 参考（交互式新手引导向导）"
title: onboard
x-i18n:
...
```bash
forge-orchestrator onboard
forge-orchestrator onboard --flow quickstart
forge-orchestrator onboard --flow manual
forge-orchestrator onboard --mode remote --remote-url ws://gateway-host:18789
```
...
- 最快开始聊天：`forge-orchestrator dashboard`（控制 UI，无需渠道设置）。
