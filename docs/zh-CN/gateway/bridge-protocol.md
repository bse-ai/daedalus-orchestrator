...
**注意：** 当前的 ForgeOrchestrator 构建不再包含 TCP bridge 监听器；本文档仅作历史参考保留。
...
- 将 bridge 绑定到 tailnet IP：在 `~/.forge-orchestrator/forge-orchestrator.json` 中设置 `bridge.bind: "tailnet"`。
