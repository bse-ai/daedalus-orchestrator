...
Deploy ForgeOrchestrator on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/forge-orchestrator/forge-orchestrator)
...
repository configures everything needed to run ForgeOrchestrator:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: FORGE_ORCH_STATE_DIR
        value: /data/.forge-orchestrator
      - key: FORGE_ORCH_WORKSPACE_DIR
        value: /data/workspace
      - key: FORGE_ORCH_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: forge-orchestrator-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/forge-orchestrator`.
...
If you use the original ForgeOrchestrator repository, Render will not auto-deploy your ForgeOrchestrator. To update it, run a manual Blueprint sync from the dashboard.
...
For ForgeOrchestrator, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any ForgeOrchestrator host.
