...
Deploy ForgeOrchestrator on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/forge-orchestrator-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/forge-orchestrator` — Control UI

## What you get

- Hosted ForgeOrchestrator Gateway + Control UI
...
- `FORGE_ORCH_STATE_DIR=/data/.forge-orchestrator` (recommended)
- `FORGE_ORCH_WORKSPACE_DIR=/data/workspace` (recommended)
- `FORGE_ORCH_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your ForgeOrchestrator state + workspace so you can migrate to another host without losing config or memory.
