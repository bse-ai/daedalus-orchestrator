---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for ForgeOrchestrator deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, ForgeOrchestrator cron scheduling for periodic checks, or version status checks on a machine running ForgeOrchestrator (laptop, workstation, Pi, VPS).
---

# ForgeOrchestrator Host Hardening

## Overview

Assess and harden the host running ForgeOrchestrator, then align it to a user-defined risk tolerance without breaking access. Use ForgeOrchestrator security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim ForgeOrchestrator changes the host firewall, SSH, or OS updates; it does not.
...
5. ForgeOrchestrator gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if ForgeOrchestrator can access sensitive data.
...
### 2) Run ForgeOrchestrator security audits (read-only)

As part of the default read-only checks, run `forge-orchestrator security audit --deep`. Only offer alternatives if the user requests them:

1. `forge-orchestrator security audit` (faster, non-probing)
2. `forge-orchestrator security audit --json` (structured output)

Offer to apply ForgeOrchestrator safe defaults (numbered):

1. `forge-orchestrator security audit --fix`

Be explicit that `--fix` only tightens ForgeOrchestrator defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check ForgeOrchestrator version/update status (read-only)

As part of the default read-only checks, run `forge-orchestrator update status`.
...
- Credential hygiene notes (location of ForgeOrchestrator creds, prefer disk encryption)
...
- ForgeOrchestrator security audit (re-run)
...
After ForgeOrchestrator install or first hardening pass, run at least one baseline audit and version check:

- `forge-orchestrator security audit`
- `forge-orchestrator security audit --deep`
- `forge-orchestrator update status`

Ongoing monitoring is recommended. Use the ForgeOrchestrator cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `forge-orchestrator cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `forge-orchestrator update status`
...
Before creating, `forge-orchestrator cron list` and match on exact `name`. If found, `forge-orchestrator cron edit <id> ...`.
If not found, `forge-orchestrator cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `forge-orchestrator update status` (preferred for source checkouts and channels)
2. `npm view forge-orchestrator version` (published npm version)

## ForgeOrchestrator command accuracy

Use only supported commands and flags:

- `forge-orchestrator security audit [--deep] [--fix] [--json]`
- `forge-orchestrator status` / `forge-orchestrator status --deep`
- `forge-orchestrator health --json`
- `forge-orchestrator update status`
- `forge-orchestrator cron add|list|runs|run`

Do not invent CLI flags or imply ForgeOrchestrator enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by ForgeOrchestrator compaction:
