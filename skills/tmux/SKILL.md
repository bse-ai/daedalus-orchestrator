...
```bash
SOCKET_DIR="${FORGE_ORCH_TMUX_SOCKET_DIR:-${FORGE_ORCH_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/forge-orchestrator-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/forge-orchestrator.sock"
SESSION=forge-orchestrator-python
...
- Use `FORGE_ORCH_TMUX_SOCKET_DIR` (legacy `FORGE_ORCH_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$FORGE_ORCH_TMUX_SOCKET_DIR/forge-orchestrator.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `FORGE_ORCH_TMUX_SOCKET_DIR`).
