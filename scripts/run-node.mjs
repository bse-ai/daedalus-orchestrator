...
const shouldBuild = () => {
  if (env.FORGE_ORCH_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message) => {
  if (env.FORGE_ORCH_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[forge-orchestrator] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["forge-orchestrator.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
