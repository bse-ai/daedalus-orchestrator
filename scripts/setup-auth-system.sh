#!/bin/bash
# Setup ForgeOrchestrator Auth Management System
...
echo "=== ForgeOrchestrator Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'forge-orchestrator-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  ForgeOrchestrator message: Send warning via ForgeOrchestrator itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/forge-orchestrator-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/forge-orchestrator-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/forge-orchestrator-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now forge-orchestrator-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status forge-orchestrator-auth-monitor.timer"
echo ""
echo "Quick commands:"
