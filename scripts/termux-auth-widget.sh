...
# ForgeOrchestrator Auth Widget for Termux
...
# Check auth status
termux-toast "Checking ForgeOrchestrator auth..."

STATUS=$(ssh "$SERVER" '$HOME/forge-orchestrator/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/forge-orchestrator/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|FORGE_ORCH_EXPIRING|FORGE_ORCH_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/forge-orchestrator/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|FORGE_ORCH_EXPIRED|FORGE_ORCH_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: ForgeOrchestrator won't work until re-auth"
                ;;
        esac
        ;;
