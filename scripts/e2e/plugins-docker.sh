...
IMAGE_NAME="forge-orchestrator-plugins-e2e"
...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  home_dir=$(mktemp -d "/tmp/forge-orchestrator-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.forge-orchestrator/extensions/demo-plugin"

  cat > "$HOME/.forge-orchestrator/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.forge-orchestrator/extensions/demo-plugin/forge-orchestrator.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$FORGE_ORCH_ENTRY" plugins list --json > /tmp/plugins.json
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/forge-orchestrator-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@forge-orchestrator/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/forge-orchestrator.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$FORGE_ORCH_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$FORGE_ORCH_ENTRY" plugins list --json > /tmp/plugins2.json
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/forge-orchestrator-plugin-dir.XXXXXX")"
...
JS
  cat > "$dir_plugin/forge-orchestrator.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$FORGE_ORCH_ENTRY" plugins install "$dir_plugin"
	  node "$FORGE_ORCH_ENTRY" plugins list --json > /tmp/plugins3.json
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/forge-orchestrator-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@forge-orchestrator/demo-plugin-file",
  "version": "0.0.1",
  "forge-orchestrator": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/forge-orchestrator.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$FORGE_ORCH_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$FORGE_ORCH_ENTRY" plugins list --json > /tmp/plugins4.json
