...
#
# Suggested cron: */30 * * * * /home/admin/forge-orchestrator/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send ForgeOrchestrator notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., forge-orchestrator-alerts)
...
STATE_FILE="$HOME/.forge-orchestrator/auth-monitor-state"
...
send_notification() {
    ...
    # Send via ForgeOrchestrator if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use forge-orchestrator
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via ForgeOrchestrator to $NOTIFY_PHONE..."
            forge-orchestrator send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: ForgeOrchestrator Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! ForgeOrchestrator is down. Run: ssh l36 '~/forge-orchestrator/scripts/mobile-reauth.sh'" "urgent"
    exit 1
