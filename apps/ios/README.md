# ForgeOrchestrator (iOS)

This is an **alpha** iOS app that connects to an ForgeOrchestrator Gateway as a `role: node`.
...
3. On iOS: ForgeOrchestrator → Settings → Gateway → paste setup code → Connect
...
1. Select the `ForgeOrchestrator` scheme
...
cd apps/ios
xcodegen generate
xcodebuild test -project ForgeOrchestrator.xcodeproj -scheme ForgeOrchestrator -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/ForgeOrchestratorKit` contains the shared transport/types used by the iOS app.
