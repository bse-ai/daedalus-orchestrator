---
description: Update ForgeOrchestrator from upstream when branch has diverged (ahead/behind)
---

# ForgeOrchestrator Upstream Sync Workflow
...
# Run diagnostics
pnpm forge-orchestrator doctor
```
...
```bash
# Kill running app
pkill -x "ForgeOrchestrator" || true

# Move old version
mv /Applications/ForgeOrchestrator.app /tmp/ForgeOrchestrator-backup.app

# Install new build
cp -R dist/ForgeOrchestrator.app /Applications/

# Launch
open /Applications/ForgeOrchestrator.app
```
...
pnpm forge-orchestrator health

# Verify no zombie processes
ps aux | grep -E "(forge-orchestrator|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm forge-orchestrator agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update forge-orchestrator.json with fallback chains
...
pnpm forge-orchestrator agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
