...
describe("normalizeLegacyConfigValues", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.FORGE_ORCH_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "forge-orchestrator-oauth-"));
    process.env.FORGE_ORCH_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.FORGE_ORCH_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
