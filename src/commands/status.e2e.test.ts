...
vi.mock("../infra/forge-orchestrator-root.js", () => ({
  resolveForgeOrchestratorPackageRoot: vi.fn().mockResolvedValue("/tmp/forge-orchestrator"),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("ForgeOrchestrator status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("forge-orchestrator status --all") ||
          l.includes("forge-orchestrator --profile isolated status --all") ||
          l.includes("forge-orchestrator status --all") ||
          l.includes("forge-orchestrator --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.FORGE_ORCH_GATEWAY_TOKEN = "abcd1234";
    try {
