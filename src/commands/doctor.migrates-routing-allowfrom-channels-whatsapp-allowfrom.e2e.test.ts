...
beforeEach(() => {
  ...
  resolveForgeOrchestratorPackageRoot.mockReset().mockResolvedValue(null);
  ...
  legacyReadConfigFileSnapshot.mockReset().mockResolvedValue({
    path: "/tmp/forge-orchestrator.json",
    exists: false,
    raw: null,
    parsed: {},
    valid: true,
...
const resolveForgeOrchestratorPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadForgeOrchestratorPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/forge-orchestrator-root.js", () => ({
  resolveForgeOrchestratorPackageRoot,
}));
...
describe("doctor command", () => {
  ...
  it("skips legacy gateway services migration", { timeout: 60_000 }, async () => {
    ...
    findLegacyGatewayServices.mockResolvedValueOnce([
      {
        platform: "darwin",
        label: "com.steipete.forge-orchestrator.gateway",
        detail: "loaded",
      },
    ]);
  ...
  it("offers to update first for git checkouts", async () => {
    ...
    const root = "/tmp/forge-orchestrator";
    resolveForgeOrchestratorPackageRoot.mockResolvedValueOnce(root);
