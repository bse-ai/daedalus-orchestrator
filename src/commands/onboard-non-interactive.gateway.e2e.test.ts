...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.FORGE_ORCH_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "forge-orchestrator");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.FORGE_ORCH_STATE_DIR = stateDir;
    process.env.FORGE_ORCH_CONFIG_PATH = path.join(stateDir, "forge-orchestrator.json");
