...
export async function requireValidConfig(runtime: RuntimeEnv): Promise<ForgeOrchestratorConfig | null> {
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Fix the config or run ${formatCliCommand("forge-orchestrator doctor")}.`);
    runtime.exit(1);
    return null;
  }
