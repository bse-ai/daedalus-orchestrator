...
describe("channels command", () => {
  ...
  it("surfaces Discord privileged intent issues in channels status output", () => {
    ...
    expect(lines.join("\n")).toMatch(/Run: (?:forge-orchestrator|forge-orchestrator)( --profile isolated)? doctor/);
  });
        ...
        telegram: [
          {
            ...
            enabled: true,
            configured: true,
            probe: { ok: true, bot: { username: "forge-orchestrator_bot" } },
          },
        ],
      },
    });
    expect(lines.join("\n")).toMatch(/bot:@forge-orchestrator_bot/);
  });
