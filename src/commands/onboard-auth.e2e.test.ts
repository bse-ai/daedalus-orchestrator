...
const requireAgentDir = () => {
  const agentDir = process.env.FORGE_ORCH_AGENT_DIR;
  if (!agentDir) {
    throw new Error("FORGE_ORCH_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.FORGE_ORCH_OAUTH_DIR;
  });

  it("writes auth-profiles.json under FORGE_ORCH_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-oauth-"));
    process.env.FORGE_ORCH_STATE_DIR = tempStateDir;
    process.env.FORGE_ORCH_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.FORGE_ORCH_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to FORGE_ORCH_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-minimax-"));
    process.env.FORGE_ORCH_STATE_DIR = tempStateDir;
    process.env.FORGE_ORCH_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.FORGE_ORCH_AGENT_DIR;
