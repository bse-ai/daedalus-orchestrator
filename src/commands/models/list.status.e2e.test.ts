...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveForgeOrchestratorAgentDir: vi.fn().mockReturnValue("/tmp/forge-orchestrator-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/forge-orchestrator-agent"),
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/forge-orchestrator-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/forge-orchestrator-agent-custom");
