...
describe("loader", () => {
  ...
  beforeEach(async () => {
    ...
    tmpDir = path.join(os.tmpdir(), `forge-orchestrator-test-${Date.now()}`);
    await fs.mkdir(tmpDir, { recursive: true });

    // Disable bundled hooks during tests by setting env var to non-existent directory
    originalBundledDir = process.env.FORGE_ORCH_BUNDLED_HOOKS_DIR;
    process.env.FORGE_ORCH_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
