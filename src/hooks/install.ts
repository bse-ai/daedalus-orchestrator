...
async function ensureForgeOrchestratorHooks(manifest: HookPackageManifest) {
...
export async function installHooksFromArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-hook-"));
...
export async function installHooksFromNpmSpec(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-hook-pack-"));
