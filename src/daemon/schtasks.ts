...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.FORGE_ORCH_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
...
export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.FORGE_ORCH_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
...
export async function installScheduledTask({
  ...
  const taskDescription =
    ...
    formatGatewayServiceDescription({
      profile: env.FORGE_ORCH_PROFILE,
      version: environment?.FORGE_ORCH_SERVICE_VERSION ?? env.FORGE_ORCH_SERVICE_VERSION,
    });
