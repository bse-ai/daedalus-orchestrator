...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.FORGE_ORCH_STATE_DIR = path.join(homeDir, ".forge-orchestrator");
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-home-"));
      const restoreHome = setTempHome(homeDir);

      const initial = await rpcReq<{ triggers: string[] }>(ws, "voicewake.get");
      expect(initial.ok).toBe(true);
      expect(initial.payload?.triggers).toEqual(["forge-orchestrator", "claude", "computer"]);
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".forge-orchestrator", "settings", "voicewake.json"), "utf8"),
  ...
  test("pushes voicewake.changed to nodes on connect and on updates", async () => {
    ...
    const setRes = await rpcReq<{ triggers: string[] }>(ws, "voicewake.set", {
      triggers: ["forge-orchestrator", "computer"],
    });
    expect(setRes.ok).toBe(true);
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const prevCanvasPort = process.env.FORGE_ORCH_CANVAS_HOST_PORT;
    process.env.FORGE_ORCH_GATEWAY_TOKEN = "secret";
    testTailnetIPv4.value = "100.64.0.1";
    testState.gatewayBind = "lan";
    const canvasPort = await getFreePort();
    testState.canvasHostPort = canvasPort;
    process.env.FORGE_ORCH_CANVAS_HOST_PORT = String(canvasPort);
    ...
    if (prevCanvasPort === undefined) {
      ...
      process.env.FORGE_ORCH_CANVAS_HOST_PORT = prevCanvasPort;
    }
  });
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    ...
    if (!configPath) {
      throw new Error("Missing FORGE_ORCH_CONFIG_PATH");
    }
