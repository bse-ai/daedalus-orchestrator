...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.FORGE_ORCH_CONFIG_PATH;
  const prevDisableCache = process.env.FORGE_ORCH_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-plugin-http-auth-test-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.FORGE_ORCH_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.FORGE_ORCH_DISABLE_CONFIG_CACHE;
    } else {
      process.env.FORGE_ORCH_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
