...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.FORGE_ORCH_SKIP_CHANNELS;
    prevSkipGmail = process.env.FORGE_ORCH_SKIP_GMAIL_WATCHER;
    process.env.FORGE_ORCH_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.FORGE_ORCH_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.FORGE_ORCH_SKIP_GMAIL_WATCHER;
    } else {
      process.env.FORGE_ORCH_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
