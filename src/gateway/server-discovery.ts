...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/forge-orchestrator/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (ForgeOrchestrator)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.FORGE_ORCH_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "forge-orchestrator");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.FORGE_ORCH_TAILNET_DNS?.trim();
