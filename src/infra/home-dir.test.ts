...
describe("resolveEffectiveHomeDir", () => {
  it("prefers FORGE_ORCH_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/forge-orchestrator-home"),
    );
  });
  ...
  it("expands FORGE_ORCH_HOME when set to ~", () => {
    const env = {
      FORGE_ORCH_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for FORGE_ORCH_HOME", () => {
    const result = resolveRequiredHomeDir(
      { FORGE_ORCH_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when FORGE_ORCH_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ FORGE_ORCH_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { FORGE_ORCH_HOME: "/srv/forge-orchestrator-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/forge-orchestrator-home")}/x`);
  });
