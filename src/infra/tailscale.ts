...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.FORGE_ORCH_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for ForgeOrchestrator. You can keep running the web gateway without it: \`${formatCliCommand("forge-orchestrator gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
