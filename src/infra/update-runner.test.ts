...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-update-"));
    await fs.writeFile(path.join(tempDir, "forge-orchestrator.mjs"), "export {};\n", "utf-8");
  });
  ...
  it("skips git update when worktree is dirty", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "forge-orchestrator", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "forge-orchestrator", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
    ...
    const { runner, calls } = createRunner({
      ...
      [`${process.execPath} ${path.join(tempDir, "forge-orchestrator.mjs")} doctor --non-interactive`]: {
        stdout: "",
      },
    });
  ...
  it("updates global npm installs when detected", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g forge-orchestrator@latest") {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
    ...
    expect(calls.some((call) => call === "npm i -g forge-orchestrator@latest")).toBe(true);
  });
  ...
  it("uses update channel for global npm installs when tag is omitted", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g forge-orchestrator@beta") {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "forge-orchestrator", version: "2.0.0" }),
          "utf-8",
    ...
    expect(result.before?.version).toBe("1.0.0");
    expect(result.after?.version).toBe("2.0.0");
    expect(calls.some((call) => call === "npm i -g forge-orchestrator@beta")).toBe(true);
  });
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(bunGlobalRoot, "forge-orchestrator");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.writeFile(
        path.join(pkgRoot, "package.json"),
        JSON.stringify({ name: "forge-orchestrator", version: "1.0.0" }),
      ...
      const runCommand = async (argv: string[]) => {
        ...
        if (key === "bun add -g forge-orchestrator@latest") {
          await fs.writeFile(
            path.join(pkgRoot, "package.json"),
            JSON.stringify({ name: "forge-orchestrator", version: "2.0.0" }),
            "utf-8",
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g forge-orchestrator@latest")).toBe(true);
    } finally {
  ...
  it("fails with a clear reason when forge-orchestrator.mjs is missing", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "forge-orchestrator", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
    ...
    await fs.rm(path.join(tempDir, "forge-orchestrator.mjs"), { force: true });
  ...
  it("repairs UI assets when doctor run removes control-ui files", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (
        key === `${process.execPath} ${path.join(tempDir, "forge-orchestrator.mjs")} doctor --non-interactive`
      ) {
