...
describe("device identity state dir defaults", () => {
  ...
  it("writes the default identity file under FORGE_ORCH_STATE_DIR", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-identity-state-"));
