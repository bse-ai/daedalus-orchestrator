...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.FORGE_ORCH_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "ForgeOrchestrator";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(ForgeOrchestrator\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.FORGE_ORCH_MDNS_HOSTNAME?.trim() ||
    process.env.FORGE_ORCH_MDNS_HOSTNAME?.trim() ||
    "forge-orchestrator";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
      .trim() || "forge-orchestrator";
  const instanceName =
    typeof opts.instanceName === "string" && opts.instanceName.trim()
      ? opts.instanceName.trim()
      : `${hostname} (ForgeOrchestrator)`;
