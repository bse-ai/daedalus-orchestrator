...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.FORGE_ORCH_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*forge-orchestrator-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "forge-orchestrator";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "forge-orchestrator");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "forge-orch-gw");
  ...
  records.push(`_forge-orch-gw._tcp IN PTR ${instanceLabel}._forge-orch-gw._tcp`);
  records.push(`${instanceLabel}._forge-orch-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._forge-orch-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; forge-orchestrator-content-hash: ${contentHash}\n${contentBody}`;
}
