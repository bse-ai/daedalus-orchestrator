    ...
    async () => {
      await withTempDir(async (tmp) => {
        ...
        await fs.writeFile(
          ...
          JSON.stringify({ name: "forge-orchestrator" }),
          "utf-8",
        );
        await fs.writeFile(
        ...
        await fs.writeFile(
          path.join(tmp, "forge-orchestrator.mjs"),
          "#!/usr/bin/env node\nif (process.argv.includes('--version')) console.log('9.9.9-test');\n",
          "utf-8",
        );
        await fs.chmod(path.join(tmp, "forge-orchestrator.mjs"), 0o755);
...
`;
        ...
        const env = {
          ...
          FORGE_ORCH_FORCE_BUILD: "1",
          FORGE_ORCH_RUNNER_LOG: "0",
        };
