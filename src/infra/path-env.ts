...
function candidateBinDirs(opts: EnsureForgeOrchestratorPathOpts): string[] {
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/forge-orchestrator` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "forge-orchestrator"))) {
