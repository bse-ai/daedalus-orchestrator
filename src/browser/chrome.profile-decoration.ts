...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".forge-orchestrator-profile-decorated");
}
 ...
 */
export function decorateForgeOrchestratorProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_FORGE_ORCH_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_FORGE_ORCH_BROWSER_COLOR).toUpperCase();
