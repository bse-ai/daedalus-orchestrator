...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(forgeOrch?.driver).toBe("forge-orchestrator");
    expect(forgeOrch?.cdpPort).toBe(18800);
    expect(forgeOrch?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from FORGE_ORCH_GATEWAY_PORT when unset", () => {
    const prev = process.env.FORGE_ORCH_GATEWAY_PORT;
    process.env.FORGE_ORCH_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const forgeOrch = resolveProfile(resolved, "forge-orchestrator");
      expect(forgeOrch?.cdpPort).toBe(19012);
      expect(forgeOrch?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.FORGE_ORCH_GATEWAY_PORT;
      } else {
        process.env.FORGE_ORCH_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const forgeOrch = resolveProfile(resolved, "forge-orchestrator");
      expect(forgeOrch?.cdpPort).toBe(19022);
      expect(forgeOrch?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.FORGE_ORCH_GATEWAY_PORT;
      } else {
