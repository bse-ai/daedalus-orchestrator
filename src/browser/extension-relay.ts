...
const RELAY_AUTH_HEADER = "x-forge-orchestrator-relay-token";
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/ForgeOrchestrator-Extension-Relay",
          revision: "0",
          userAgent: "ForgeOrchestrator-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "ForgeOrchestrator/extension-relay",
