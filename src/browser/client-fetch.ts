...
function withLoopbackBrowserAuth(
  ...
  try {
    ...
    if (auth.token) {
      ...
      headers.set("x-forge-orchestrator-password", auth.password);
    }
  } catch {
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  const hint = isAbsoluteHttp(url)
    ...
    : `Start (or restart) the ForgeOrchestrator gateway (ForgeOrchestrator.app menubar, or \`${formatCliCommand("forge-orchestrator gateway")}\`) and try again.`;
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the ForgeOrchestrator browser control service (timed out after ${timeoutMs}ms). ${hint}`,
    );
  }
  return new Error(`Can't reach the ForgeOrchestrator browser control service. ${hint} (${msg})`);
}
