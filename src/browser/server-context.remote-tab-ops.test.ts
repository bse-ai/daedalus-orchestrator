...
function makeState(
  profile: "remote" | "forge-orchestrator",
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const state = makeState("forge-orchestrator");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const forgeOrch = ctx.forProfile("forge-orchestrator");

    const opened = await forgeOrch.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("forge-orchestrator")?.lastTargetId).toBe("CREATED");
  });
});
