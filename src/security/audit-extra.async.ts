...
import type { ForgeOrchestratorConfig, ConfigFileSnapshot } from "../config/config.js";
...
function listWorkspaceDirs(cfg: ForgeOrchestratorConfig): string[] {
...
export async function collectPluginsTrustFindings(params: {
  ...
  if (enabledExtensionPluginIds.length > 0) {
    ...
    for (const context of contexts) {
      ...
      const broadPolicy = isToolAllowedByPolicies("__forgeOrch_plugin_probe__", policies);
...
export async function collectPluginsCodeSafetyFindings(params: {
  ...
  const entries = await fs.readdir(extensionsDir, { withFileTypes: true }).catch((err) => {
    findings.push({
      ...
      remediation:
        "Check file permissions and plugin layout, then rerun `forge-orchestrator security audit --deep`.",
    });
    return [];
  });
