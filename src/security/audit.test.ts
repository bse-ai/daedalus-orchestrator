...
describe("security audit", () => {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-security-audit-win-"));
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "forge-orchestrator.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.FORGE_ORCH_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-security-audit-discord-"));
    process.env.FORGE_ORCH_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prevStateDir == null) {
        ...
        process.env.FORGE_ORCH_STATE_DIR = prevStateDir;
      }
    }
  });
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "forge-orchestrator-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.FORGE_ORCH_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.FORGE_ORCH_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-security-audit-discord-open-"));
    process.env.FORGE_ORCH_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ForgeOrchestratorConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.FORGE_ORCH_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-security-audit-slack-"));
    process.env.FORGE_ORCH_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ForgeOrchestratorConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.FORGE_ORCH_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-security-audit-slack-open-"));
    process.env.FORGE_ORCH_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ForgeOrchestratorConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.FORGE_ORCH_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-security-audit-telegram-"));
    process.env.FORGE_ORCH_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ForgeOrchestratorConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: ForgeOrchestratorConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    ...
    process.env.FORGE_ORCH_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: ForgeOrchestratorConfig = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.forge-orchestrator",
      configPath: "/Users/test/Dropbox/.forge-orchestrator/forge-orchestrator.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-security-audit-"));
    const stateDir = path.join(tmp, "state");
    ...
    try {
      const cfg: ForgeOrchestratorConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "forge-orchestrator.json"),
      });
  ...
  it("flags enabled extensions when tool policy can expose plugin tools", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-security-audit-plugins-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(path.join(stateDir, "extensions", "some-plugin"), {
      recursive: true,
      mode: 0o700,
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-audit-scanner-"));
...
`,
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.FORGE_ORCH_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.FORGE_ORCH_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.FORGE_ORCH_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.FORGE_ORCH_GATEWAY_PASSWORD;
      } else {
        process.env.FORGE_ORCH_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env password over remote password", async () => {
      process.env.FORGE_ORCH_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: ForgeOrchestratorConfig = {
        gateway: {
          mode: "remote",
