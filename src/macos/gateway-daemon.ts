...
const BUNDLED_VERSION =
  (typeof __FORGE_ORCH_VERSION__ === "string" && __FORGE_ORCH_VERSION__) ||
  process.env.FORGE_ORCH_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `forge-orchestrator --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.FORGE_ORCH_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.FORGE_ORCH_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
    bindRaw === "loopback" ||
...
void main().catch((err) => {
  console.error(
    "[forge-orchestrator] Gateway daemon failed:",
