...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/forge-orchestrator-telegram-reply-threading-${Math.random()
    .toString(16)
    .slice(2)}.json`,
}));
...
describe("createTelegramBot", () => {
  ...
  it("honors routed group activation from session store", async () => {
    ...
    const replySpy = replyModule.__replySpy as unknown as ReturnType<typeof vi.fn>;
    replySpy.mockReset();
    const storeDir = fs.mkdtempSync(path.join(os.tmpdir(), "forge-orchestrator-telegram-"));
