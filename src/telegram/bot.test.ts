...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/forge-orchestrator-telegram-bot-${Math.random().toString(16).slice(2)}.json`,
}));
...
describe("createTelegramBot", () => {
  ...
  it("requests pairing by default for unknown DM senders", async () => {
    ...
    expect(pairingText).toContain("forge-orchestrator pairing approve telegram PAIRME12");
  ...
  it("blocks group messages when telegram.groups is set without a wildcard", async () => {
    ...
    await handler({
      message: {
        ...
        text: "@forge-orchestrator_bot hello",
        date: 1736380800,
      },
      me: { username: "forge-orchestrator_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
  ...
  it("honors routed group activation from session store", async () => {
    ...
    const replySpy = replyModule.__replySpy as unknown as ReturnType<typeof vi.fn>;
    replySpy.mockReset();
    const storeDir = createTempDir("forge-orchestrator-telegram-");
