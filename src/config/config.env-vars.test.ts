...
describe("config env vars", () => {
  ...
  it("loads ${VAR} substitutions from ~/.forge-orchestrator/.env on repeated runtime loads", async () => {
    await withTempHome(async (home) => {
      await withEnvOverride(
        {
          FORGE_ORCH_STATE_DIR: path.join(home, ".forge-orchestrator"),
          FORGE_ORCH_STATE_DIR: undefined,
          FORGE_ORCH_HOME: undefined,
          FORGE_ORCH_HOME: undefined,
          BRAVE_API_KEY: undefined,
          FORGE_ORCH_DISABLE_CONFIG_CACHE: "1",
        },
        async () => {
