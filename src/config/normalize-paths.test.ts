  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.forge-orchestrator/logs/forge-orchestrator.log" },
        hooks: {
          path: "~/.forge-orchestrator/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.forge-orchestrator/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".forge-orchestrator", "logs", "forge-orchestrator.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".forge-orchestrator", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".forge-orchestrator", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".forge-orchestrator", "agents", "main"));
