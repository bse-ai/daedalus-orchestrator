...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real forge-orchestrator.json.
...
describe("rotateSessionFile", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-rotate-"));
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-pruning-integ-"));
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.FORGE_ORCH_SESSION_CACHE_TTL_MS;
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      ...
      process.env.FORGE_ORCH_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
