...
/**
 * Test for issue #6070:
 * `forge-orchestrator config set` should use snapshot.parsed (raw user config) instead of
...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-config-cli-"));
  const originalEnv = { ...process.env };
  try {
    // Override config path to use temp directory
    process.env.FORGE_ORCH_CONFIG_PATH = path.join(home, ".forge-orchestrator", "forge-orchestrator.json");
