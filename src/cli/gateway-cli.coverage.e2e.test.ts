...
describe("gateway-cli coverage", () => {
  ...
  it("registers gateway discover and prints JSON", async () => {
    ...
    discoverGatewayBeacons.mockResolvedValueOnce([
      {
        instanceName: "Studio (ForgeOrchestrator)",
        displayName: "Studio",
        domain: "local.",
        host: "studio.local",
  ...
  it("registers gateway discover and prints human output with details on new lines", async () => {
    ...
    discoverGatewayBeacons.mockResolvedValueOnce([
      {
        ...
        host: "studio.forge-orchestrator.internal",
        lanHost: "studio.local",
        tailnetDns: "studio.tailnet.ts.net",
        gatewayPort: 18789,
        sshPort: 22,
    ...
    expect(out).toContain("- Studio forge-orchestrator.internal.");
    expect(out).toContain("  tailnet: studio.tailnet.ts.net");
    expect(out).toContain("  host: studio.forge-orchestrator.internal");
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ FORGE_ORCH_GATEWAY_PORT: "19001" }, async () => {
      runtimeLogs.length = 0;
      runtimeErrors.length = 0;
