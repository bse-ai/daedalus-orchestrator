...
const resolveDevWorkspaceDir = (env: NodeJS.ProcessEnv = process.env): string => {
  ...
  const profile = env.FORGE_ORCH_PROFILE?.trim().toLowerCase();
  if (profile === "dev") {
    return baseDir;
  }
...
async function ensureDevWorkspace(dir: string) {
  ...
  const [agents, soul, tools, identity, user] = await Promise.all([
    loadDevTemplate(
      "AGENTS.dev.md",
      `# AGENTS.md - ForgeOrchestrator Dev Workspace\n\nDefault dev workspace for forge-orchestrator gateway --dev.\n`,
    ),
    loadDevTemplate(
      "SOUL.dev.md",
