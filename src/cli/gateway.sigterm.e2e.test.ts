...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "forge-orchestrator-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      FORGE_ORCH_NO_RESPAWN: "1",
      FORGE_ORCH_STATE_DIR: stateDir,
      FORGE_ORCH_SKIP_CHANNELS: "1",
      FORGE_ORCH_SKIP_GMAIL_WATCHER: "1",
      FORGE_ORCH_SKIP_CRON: "1",
      FORGE_ORCH_SKIP_BROWSER_CONTROL_SERVER: "1",
      FORGE_ORCH_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "forge-orchestrator-entry-bootstrap.mjs");
