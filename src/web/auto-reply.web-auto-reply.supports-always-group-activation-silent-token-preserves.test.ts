...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-web-home-"));
...
const makeSessionStore = async (
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-session-"));
...
describe("web auto-reply", () => {
  ...
  it("ignores JID mentions in self-chat mode (group chats)", async () => {
    ...
    setLoadConfigMock(() => ({
      ...
      messages: {
        groupChat: {
          mentionPatterns: ["\\bforge-orchestrator\\b"],
        },
      },
    }));
    ...
    // Text-based mentionPatterns still work (user can type "forge-orchestrator" explicitly).
    await capturedOnMessage?.({
      body: "forge-orchestrator ping",
      from: "123@g.us",
      conversationId: "123@g.us",
      chatId: "123@g.us",
      chatType: "group",
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/forge-orchestrator-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/forge-orchestrator-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
