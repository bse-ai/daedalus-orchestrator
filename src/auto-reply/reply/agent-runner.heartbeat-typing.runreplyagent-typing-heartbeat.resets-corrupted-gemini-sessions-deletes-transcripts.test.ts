...
describe("runReplyAgent typing (heartbeat)", () => {
  it("resets corrupted Gemini sessions and deletes transcripts", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "forge-orchestrator-session-reset-"));
  ...
  it("keeps sessions intact on other errors", async () => {
    const prevStateDir = process.env.FORGE_ORCH_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "forge-orchestrator-session-noreset-"));
    process.env.FORGE_ORCH_STATE_DIR = stateDir;
    try {
