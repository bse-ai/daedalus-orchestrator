...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_FORGE_ORCH_BROWSER_PROFILE_NAME,
    profiles: {
      [DEFAULT_FORGE_ORCH_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (!state.exists) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: { "forge-orchestrator.sandboxBrowser": "1" },
    });
    ...
    args.push("-e", `FORGE_ORCH_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `FORGE_ORCH_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `FORGE_ORCH_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `FORGE_ORCH_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `FORGE_ORCH_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
