...
describe("loadExtraBootstrapFiles", () => {
  it("loads recognized bootstrap files from glob patterns", async () => {
    const workspaceDir = await makeTempWorkspace("forge-orchestrator-extra-bootstrap-glob-");
  ...
  it("keeps path-traversal attempts outside workspace excluded", async () => {
    const rootDir = await makeTempWorkspace("forge-orchestrator-extra-bootstrap-root-");
  ...
  it("supports symlinked workspace roots with realpath checks", async () => {
    ...
    const rootDir = await makeTempWorkspace("forge-orchestrator-extra-bootstrap-symlink-");
