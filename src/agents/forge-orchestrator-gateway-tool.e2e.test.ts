...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.FORGE_ORCH_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-test-"));
    ...
    try {
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: forge-orchestrator --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/forge-orchestrator" } }\n}\n';
