...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("forge-orchestrator-ws-", async (workspaceDir) => {
      await withTempDir("forge-orchestrator-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createForgeOrchestratorCodingTools({ workspaceDir });
      ...
      await withTempDir("forge-orchestrator-cwd-", async (otherDir) => {
        ...
        try {
          ...
          await editTool?.execute("ws-edit", {
            path: testFile,
            oldText: "world",
            newText: "forge-orchestrator",
          });

          const updated = await fs.readFile(path.join(workspaceDir, testFile), "utf8");
          expect(updated).toBe("hello forge-orchestrator");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("forge-orchestrator-ws-", async (workspaceDir) => {
      await withTempDir("forge-orchestrator-override-", async (overrideDir) => {
        const tools = createForgeOrchestratorCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("forge-orchestrator-sandbox-", async (sandboxDir) => {
      await withTempDir("forge-orchestrator-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "forge-orchestrator-sbx-test",
        ...
        const tools = createForgeOrchestratorCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
