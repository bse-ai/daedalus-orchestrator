...
const fixtureRoot = path.join(os.tmpdir(), `forge-orchestrator-plugin-${randomUUID()}`);
let tempDirIndex = 0;
...
describe("loadForgeOrchestratorPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadForgeOrchestratorPlugins({
      cache: false,
      _skipSourcePathChecks: true,
      config: {
        plugins: {
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@forge-orchestrator/memory-core",
        version: "1.2.3",
