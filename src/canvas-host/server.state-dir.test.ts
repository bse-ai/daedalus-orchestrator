...
describe("canvas host state dir defaults", () => {
  ...
  it("uses FORGE_ORCH_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("ForgeOrchestrator Canvas");
    } finally {
      await handler.close();
