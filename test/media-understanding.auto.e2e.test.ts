...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("forge-orchestrator-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("forge-orchestrator-bin-sherpa-");
      const modelDir = await makeTempDir("forge-orchestrator-sherpa-model-");
      ...
      const cfg: ForgeOrchestratorConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("forge-orchestrator-bin-whispercpp-");
      const modelDir = await makeTempDir("forge-orchestrator-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("forge-orchestrator-bin-gemini-");
      ...
      const cfg: ForgeOrchestratorConfig = { tools: { media: { image: {} } } };
