# ForgeOrchestrator ACP Bridge

This document describes how the ForgeOrchestrator ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`forge-orchestrator acp` exposes an ACP agent over stdio and forwards prompts to a running
ForgeOrchestrator Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `forge-orchestrator acp` and speaks ACP messages over stdio.
...
forge-orchestrator acp --reset-session
```
...
`forge-orchestrator acp` resolves the Gateway URL and auth from CLI flags or config:
