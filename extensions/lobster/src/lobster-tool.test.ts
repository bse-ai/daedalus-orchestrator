...
import type { ForgeOrchestratorPluginApi, ForgeOrchestratorPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "forge-orchestrator-lobster-plugin-") {
...
describe("lobster plugin tool", () => {
  ...
  it("tolerates noisy stdout before the JSON envelope", async () => {
    ...
    const { dir } = await writeFakeLobsterScript(
      `const payload = ${JSON.stringify(payload)};\n` +
        ...
        `process.stdout.write(JSON.stringify(payload));\n`,
      "forge-orchestrator-lobster-plugin-noisy-",
    );
  ...
  it("rejects invalid JSON from lobster", async () => {
    const { dir } = await writeFakeLobsterScript(
      ...
      "forge-orchestrator-lobster-plugin-bad-",
    );
