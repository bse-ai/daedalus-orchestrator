...
ForgeOrchestrator extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
forge-orchestrator channels login --channel zalouser              # Login via QR
forge-orchestrator channels login --channel zalouser --account work
...
forge-orchestrator directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
forge-orchestrator message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
forge-orchestrator message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `forge-orchestrator channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
forge-orchestrator channels login --channel zalouser --account work
forge-orchestrator message send --channel zalouser --account work --target <id> --message "Hello"
