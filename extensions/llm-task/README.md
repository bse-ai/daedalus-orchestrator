...
`forge-orchestrator.invoke --each`) without adding new ForgeOrchestrator code per workflow.
...
This extension depends on ForgeOrchestrator internal modules (the embedded agent runner).
It is intended to ship as a **bundled** ForgeOrchestrator extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.forge-orchestrator/extensions` as a standalone plugin directory.
