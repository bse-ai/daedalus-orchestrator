...
// NOTE: This extension is intended to be bundled with ForgeOrchestrator.
// When running from source (tests/dev), ForgeOrchestrator internals live under src/.
...
export function createLlmTaskTool(api: ForgeOrchestratorPluginApi) {
  return {
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-llm-task-"));
