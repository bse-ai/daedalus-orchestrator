...
import { approveDevicePairing, listDevicePairing } from "forge-orchestrator/plugin-sdk";
...
function resolveGatewayPort(cfg: ForgeOrchestratorPluginApi["config"]): number {
  const envRaw =
    process.env.FORGE_ORCH_GATEWAY_PORT?.trim() || process.env.FORGE_ORCH_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: ForgeOrchestratorPluginApi): Promise<string | null> {
...
async function resolveGatewayUrl(api: ForgeOrchestratorPluginApi): Promise<ResolveUrlResult> {
