...
metadata: { "forge-orchestrator": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == ForgeOrchestrator `sessions_spawn`
- **File I/O** == ForgeOrchestrator `read`/`write`
- **Remote fetch** == ForgeOrchestrator `web_fetch` (or `exec` with curl when POST is required)
