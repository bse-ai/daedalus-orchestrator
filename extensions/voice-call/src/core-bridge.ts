...
function resolveForgeOrchestratorRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.FORGE_ORCH_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["forge-orchestrator"]) {
  ...
  throw new Error("Unable to resolve core root. Set FORGE_ORCH_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveForgeOrchestratorRoot(), "dist", "extensionAPI.js");
