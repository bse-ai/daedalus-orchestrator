...
- Security/Browser: constrain `POST /trace/stop`, `POST /wait/download`, and `POST /download` output paths to ForgeOrchestrator temp roots and reject traversal/escape paths.
...
- Security/Onboarding: clarify multi-user DM isolation remediation with explicit `forge-orchestrator config set session.dmScope ...` commands in security audit, doctor security, and channel onboarding guidance. (#13129) Thanks @VintLin.
...
- OpenAI Codex/Auth: bridge ForgeOrchestrator OAuth profiles into `pi` `auth.json` so model discovery and models-list registry resolution can use Codex OAuth credentials. (#15184) Thanks @loiie45e.
...
- Config: preserve `${VAR}` env references when writing config files so `forge-orchestrator config set/apply/patch` does not persist secrets to disk. Thanks @thewilloftheshadow.
...
- Gateway/Control UI: resolve missing dashboard assets when `forge-orchestrator` is installed globally via symlink-based Node managers (nvm/fnm/n/Homebrew). (#14919) Thanks @aynorica.
...
- Signal: render mention placeholders as `@uuid`/`@phone` so mention gating and ForgeOrchestrator targeting work. (#2013) Thanks @alexgleason.
...
- Paths: structurally resolve `FORGE_ORCH_HOME`-derived home paths and fix Windows drive-letter handling in tool meta shortening. (#12125) Thanks @mcaxtr.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@forge-orchestrator/msteams` via `forge-orchestrator plugins install @forge-orchestrator/msteams`.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.
